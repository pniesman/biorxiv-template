...
// Fonts.
#let font-sans-serif = "Libertinus Sans"
#let font-serif = "Libertinus Serif"// 

// Colours.
#let zebra-gray     = rgb(luma(95.25%)) // rgb("#f3f3f3")
#let biorxiv-red  = rgb("#a93b45")    // summary title red, table red
#let biorxiv-grey = rgb("#b3b3b3")    // supertitle grey

// --- [ tables ] --------------------------------------------------------------

#let toprule(color: biorxiv-red) = {
	table.hline(stroke: 0.7pt + color)
}

#let bottomrule(color: biorxiv-red) = {
	table.hline(stroke: 0.7pt + color)
}

#let midrule(color: biorxiv-red) = {
	table.hline(stroke: 0.5pt + color)
}

#let tabsubhdr(content, colspan: 3, color: biorxiv-red) = table.header(
	level: 2, // sub header
	table.cell(
		colspan: colspan,
		inset: (y: 5pt),
...
#let summary(body) = {
	place(
		...
		#set text(font: font-serif, size: 9.9pt)
		#set par(
...
#let print-supertitle(supertitle) = {
	set text(font: font-sans-serif, size: 13.9pt, weight: "extrabold", tracking: -0.1pt, fill: biorxiv-grey)
...
#let template(
			...
			if calc.odd(here().page()) [
				// odd page
					...
					#grid(
						...
						align: (left),
						[
							#place(top+left, dx: 0.3mm, dy: -12mm)[#image("/biorxiv-template/inc/biorxiv_logo.png")]
						],
					)
					#place(
						top+left,
						dy: 1.475cm,
						dx: -2.485cm,
						rect(width: 2cm, height: 1.2cm, fill: biorxiv-grey),
					)
				] else [
					#grid(
						columns: (1fr, 1fr),
						align: (left, right),
						[
							#block(text(font: font-sans-serif, size: 19pt, weight: "bold", tracking: -1pt, fill: biorxiv-grey)[#journal])
							#v(-2mm)
							#block(text(font: font-sans-serif, size: 13.9pt, weight: "bold", fill: biorxiv-grey)[#supertitle])
						])
				]
			] else [
				// even page
	...
	set text(font: font-serif, size: 8.4pt)

	// Set link colour.
	show ref: set text(fill: biorxiv-red)
	show link: set text(fill: biorxiv-red)
	show cite: set text(fill: biorxiv-red)
	show footnote: set text(fill: biorxiv-red)
	...
	show heading.where(level: 1): it => {
		set text(font: font-sans-serif, size: 8.4pt, weight: "extrabold", tracking: -0.2pt, fill: biorxiv-red)
		// TODO: skip scale when extrabold exists (using variable fonts: https://github.com/typst/typst/issues/185)
